...
async function loadSettings() {
  try {
    ...
    if (result.aiInputEnhancerSettings) {
      currentSettings = {
        ...DEFAULT_SETTINGS,
        ...result.aiInputEnhancerSettings,
      };
    }
  } catch (error) {
...
function checkAIStatus() {
  ...
  if (statusIndicator) {
    ...
    if (typeof chrome !== "undefined" && chrome.runtime) {
      ...
      showStatusMessage(
        "Chrome browser with API Access required for AI features",
        "warning"
      );
    }
  }
...
function setupEventListeners() {
  ...
  const resetBtn = document.getElementById("resetLanguages");

  if (addBtn) {
    addBtn.addEventListener("click", showLanguageSelector);
  }

  if (resetBtn) {
    resetBtn.addEventListener("click", resetLanguages);
  }

  // Demo button
  const demoBtn = document.getElementById("tryDemoBtn");
  if (demoBtn) {
    demoBtn.addEventListener("click", openDemoPage);
  }
}
...
function removeLanguage(languageCode) {
  currentSettings.languages = currentSettings.languages.filter(
